...
import { Calendar, Clock, MapPin, Filter, Search, Sparkles, AlertTriangle, X, Star } from 'lucide-react'
...
const FEATURED_KEYS = new Set(
  [...tariqaEvents2025_2026, ...tariqaEvents2026_2027]
    .filter(e => e.featured)
    .map(e => `${e.title}|${e.date}`)
)

const isFeaturedEvent = (event) => event.featured || FEATURED_KEYS.has(`${event.title}|${event.date}`)
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-500">{t('tariqa.loading')}</div>
      </div>
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="mt-4 text-sm text-gray-600 text-center">
                {filteredEvents.length} {t('tariqa.eventsFound')}
              </div>
            </div>
          </motion.div>
                ...
                {futureEvents.map((event, index) => {
                  ...
                  return (
                    ...
                    >
                      {featured && (
                        <motion.div
                          className="absolute -inset-1 rounded-2xl bg-gradient-to-r from-gold-400 via-amber-300 to-gold-500 blur-md"
                      ...
                      <div
                        className={`group relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 ${
                          featured
                            ? 'bg-gradient-to-br from-gold-50 via-amber-50 to-white'
                      ...
                      >
                        ...
                        <div className={`h-2 ${
                          conflict
                            ...
                            : featured
                            ? 'bg-gradient-to-r from-gold-400 via-amber-500 to-gold-600'
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex justify-between items-start mb-4">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold text-white ${
                              featured ? 'bg-gradient-to-r from-gold-500 to-amber-600' : 'bg-emerald-600'
                            }`}>
                              {featured ? <Star className="w-3 h-3 mr-1" /> : <Sparkles className="w-3 h-3 mr-1" />}
                              {featured ? t('tariqa.featuredBadge') : t('tariqa.upcomingBadge')}
                            </span>
                            {event.category && (
                          ...
                          {/* Date */}
                          <div className={`mb-4 p-3 bg-white rounded-xl border-2 ${featured ? 'border-gold-300' : 'border-emerald-200'}`}>
                            <div className="flex items-center gap-2">
                              <Calendar className={`w-5 h-5 ${featured ? 'text-amber-600' : 'text-emerald-600'}`} />
                              <p className={`text-sm font-semibold ${featured ? 'text-amber-800' : 'text-emerald-800'}`}>
                          ...
                          <button
                            ...
                            className={`mt-4 w-full flex items-center justify-center gap-2 px-4 py-2 text-white rounded-xl transition-colors ${
                              featured ? 'bg-gradient-to-r from-gold-500 to-amber-600 hover:from-gold-600 hover:to-amber-700' : 'bg-emerald-600 hover:bg-emerald-700'
                            }`}
                          >
                            <span className="text-sm font-semibold">{t('tariqa.viewDetails')}</span>
                          </button>
                        </div>
          ...
          {filteredEvents.length === 0 && (
            ...
            >
              ...
              <p className="text-gray-500 text-lg">{t('tariqa.noResults')}</p>
              <p className="text-gray-400 text-sm mt-2">{t('tariqa.modifyFilters')}</p>
            </motion.div>
          )}
        </div>
      </section>
            ...
            <div className="p-8">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <div className="flex items-center gap-3 p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                  ...
                  <div>
                    <p className="text-xs text-emerald-600 font-medium uppercase">{t('hadara.date') || 'Date'}</p>
                ...
                {selectedEvent.location && (
                  <div className="flex items-center gap-3 p-4 bg-purple-50 rounded-xl border border-purple-100">
                    ...
                    <div>
                      <p className="text-xs text-purple-600 font-medium uppercase">{t('tariqa.location')}</p>
